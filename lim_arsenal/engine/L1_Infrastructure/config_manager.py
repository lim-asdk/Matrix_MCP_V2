...
class ConfigManager:
    ...
    def load(self):
        ...
        if self.config_file.exists():
            try:
                ...
                for profile in self.config.get("profiles", []):
                    inline_key = profile.pop("api_key", None)
                    if not inline_key or inline_key == "******":
                        continue
                    safe_id = re.sub(r"[^a-zA-Z0-9._-]", "_", profile.get("id") or profile.get("name") or "profile")
                    key_file = profile.get("key_file") or f"{safe_id}.json"
                    profile["key_file"] = key_file
                    key_path = self.profile_dir / key_file
                    try:
                        key_path.write_text(json.dumps({"api_key": inline_key}, ensure_ascii=False), encoding="utf-8")
                        migrated_keys = True
                    except Exception as e:
                        logger.error(f"Failed to migrate inline API key for {profile.get('id', safe_id)}: {e}")
                if migrated_keys:
                    self.save()
            except Exception as e:
        ...
        for s_file in self.server_dir.glob("*.json"):
            try:
                ...
                logger.info(f"[Config] SERVER LOADED: {s_id} from {s_file}")
            except Exception as e:
                logger.error(f"서버 설정 로드 실패 ({s_file.name}): {e}")
        
        legacy_mcp_config = PathManager.PROJECT_ROOT / "mcp_config.json"
        if legacy_mcp_config.exists():
            try:
                legacy_data = json.loads(legacy_mcp_config.read_text(encoding="utf-8-sig"))
                for s_id, s_data in legacy_data.get("mcpServers", {}).items():
                    if s_id not in self.config["mcpServers"]:
                        self.config["mcpServers"][s_id] = s_data
                        logger.info(f"[Config] LEGACY SERVER LOADED: {s_id} from {legacy_mcp_config}")
            except Exception as e:
                logger.error(f"[Config] Failed to load legacy mcp_config.json: {e}")

        logger.info(f"[Config] TOTAL SERVERS LOADED: {len(self.config['mcpServers'])}")
    ...
    def save_server(self, server_id, server_data):
        ...
        try:
            ...
            logger.info(f"[Config] SERVER SAVED SUCCESS: {s_file}")
            logger.info(f"[Config] CURRENT SERVER_DIR: {self.server_dir}")
            return True
        except Exception as e:
            logger.error(f"[Config] SERVER SAVE FAILED ({server_id}): {e}")
            import traceback
